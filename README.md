...
FastAPI-сервис для аналитических расчетов энергопотребления. Первый сценарий сервиса сравнивает два непересекающихся периода и выполняет двухсторонний z-тест гипотезы:
...
То есть альтернативная гипотеза именно двухсторонняя: сервис проверяет, отличаются ли средние значения в любую сторону. Он не проверяет гипотезу вида `μ2 > μ1` или `μ1 > μ2`.

Из-за двухстороннего теста критическое значение берется по правилу:
...
Например, при `alpha = 0.05` используется табличное значение около `1.96`. Для односторонней гипотезы "среднее второго периода больше среднего первого" критическое значение было бы другим, около `1.65`, но такой сценарий сейчас не реализован.
